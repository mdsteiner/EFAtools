...
Rcpp::List rotate_cf_orth(const arma::mat& L,
  ...
  // Distinct local minima, counted on the optimized objective rounded to eps, as in
  // GPArotation's randomStarts diagnostics.
  std::set<long long> distinct;
  for (double v : summary.all_values) {
    if (std::isfinite(v)) {
      distinct.insert(std::llround(v / eps));
    }
  }
  int n_distinct_minima = static_cast<int>(distinct.size());
  ...
  return Rcpp::List::create(
    ...
    Rcpp::Named("method") = "cf_orthogonal",
    ...
    Rcpp::Named("n_distinct_minima") = n_distinct_minima
  );
}
