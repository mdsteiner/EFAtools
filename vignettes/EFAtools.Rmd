...
```{r}
efa_retain(DOSPERT_sub,
           criteria = c("PARALLEL", "EKC", "KGC", "SCREE", "SMT", "MAP"))
```
...
across all domains except the social one, whose items load much less strongly on it — that
is, the non-social domains covary considerably, while the social domain stands somewhat
apart.
...
highest-loading factor. With no cross-loadings to worry about here, it is easiest to leave
`factor_map` unspecified:
