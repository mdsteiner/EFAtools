...
names instead (`EFA()`, `N_FACTORS()`, `OMEGA()`, ...). The uppercase names have **not**
still work, but the `efa_*` functions are now the recommended interface. This vignette explains the change, gives the full old-to-new
mapping, and walks through the one migration that involves more than a rename: `EFA()` to
`efa_fit()`.
