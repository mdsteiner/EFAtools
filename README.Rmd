...
* **Screening and suitability**: `efa_screen()` checks the data for multivariate normality, outliers, and suitability for factor analysis. `efa_kmo()` and `efa_bartlett()` run the Kaiser-Meyer-Olkin criterion and Bartlett's test of sphericity separately.
...
The uppercase names (`EFA()`, `N_FACTORS()`, ...) still work and keep their arguments. Use the `efa_*` names for new code.
