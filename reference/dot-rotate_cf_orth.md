...
Rotate a loading matrix orthogonally under the Crawford-Ferguson
criterion using a gradient-projection optimizer along the orthogonal
(Stiefel) manifold.
...
orthogonal group via a polar (singular value) projection. `kappa = 0` is
the quartimax criterion and `kappa = ncol(A) / (2 * nrow(A))` is the
equamax criterion.
...
Crawford, C. B., & Ferguson, G. A. (1970). A general rotation criterion
