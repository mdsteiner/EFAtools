...
…) are superseded by their `efa_*` equivalents, but remain exported and
keep their arguments, so existing code keeps working.
...
Use
[`efa_retain()`](https://mdsteiner.github.io/EFAtools/reference/efa_retain.md)
to test the suitability of the data for factor analysis and to run
multiple factor retention criteria with a single function call.
