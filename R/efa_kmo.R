...
#'
#' @seealso [efa_bartlett()] for another measure to determine
...
efa_kmo <- function(x, use = c("pairwise.complete.obs", "all.obs", "complete.obs",
  ...
  # the trailing legacy class is load-bearing: it keeps `inherits(x, "KMO")`
  # working in code written against the superseded name. Do not drop it.
  class(output) <- c("efa_kmo", "KMO")
