...
#' @param n_fac_EFA numeric. Number of factors to extract if
...
KGC <- function(x, eigen_type = c("PCA", "SMC", "EFA"),
                use = c("pairwise.complete.obs", "all.obs", "complete.obs",
                        "everything", "na.or.complete"), n_fac_EFA = 1, ...){
  ...
  use <- match.arg(use)
  checkmate::assert_count(n_fac_EFA)
  ...
  if("EFA" %in% eigen_type) {
    ...
    EFA_h2 <- EFA(R, n_factors = n_fac_EFA, ...)$h2
    diag(R) <- EFA_h2
